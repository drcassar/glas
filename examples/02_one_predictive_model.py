'''
This script searches for glasses having a high refractive index. No constraints
are applied, so the candidates most probably are not glass-formers.
...
class Searcher(GLAS):
    ...
    def fitness_function(self, population):
        ...
        In this problem we want to find a glass with high refractive index. Note
        that no constraints on glass forming ability are present, so the
        selected candidates will most probably not form a glass.

        '''
        atomic_array = population @ self.conversion_matrix
